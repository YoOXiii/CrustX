...
float4 frag (Interpolate i) : SV_Target
{
    ...
    //     float4 diffuseIBL = tex2Dlod( _DiffuseIBL, float4(DirToRectilinear(i.normal), 0, 0 ) ); // pre-fetch the diffuse IBL
    ...
    float3 V =  normalize( _WorldSpaceCameraPos - i.wPos );
        ...
        float4 specularIBL = tex2Dlod( _SpecularIBL, float4(DirToRectilinear(viewRefl), mip, mip ) ); // pre-fetch the diffuse IBL
